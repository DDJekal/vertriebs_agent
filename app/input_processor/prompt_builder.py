...
TALENT_REPORT_POSTAMBLE = (
    ...
    "\n"
    "**REGIONALE STATISTIKEN (Folie 3) – PFLICHT:**\n"
    "- Die 4 `regional_stats` beziehen sich auf **verfügbare Personen mit der "
    "Zielgruppe** im 40-km-Umkreis (Fachkräfte-Pool), NICHT auf offene Stellen. "
    "Beispiele: Anzahl Fachkräfte mit der gesuchten Qualifikation in der Region, "
    "davon aktiv stellensuchend/wechselwillig, Arbeitgeber in der Branche, Trend.\n"
    "\n"
    "**CHANCEN (Folie 6) – PFLICHT:**\n"
    "- Wähle 3 passende Dienstleistungen aus dem Pool in der SKILL.md "
    "(Social Recruiting Agentur, Employer Branding Agentur, Performance Recruiting Agentur, "
    "Personalmarketing Agentur, Employer Branding Video, Recruiting Filme, "
    "Pflegepersonal finden, Mitarbeiter finden im Handwerk). "
    "Branchenspezifische Dienste haben Vorrang vor allgemeinen. "
    "Bullets müssen konkret auf die Schwächen der Wettbewerber und Bedürfnisse "
    "der Zielgruppe eingehen – keine generischen Floskeln.\n"
    "\n"
    "**LOGO – PFLICHT:**\n"
    "- Footer ist dunkelgrün (#1a2e0d) – das Logo muss HELL sein (weiss/hellgrün).\n"
    "- Das Skript sucht zuerst nach `logo_white.png` oder `logo_white.svg` im Projektordner, "
    "dann im Skill `templates/`, Fallback: `logo.png`/`logo.svg`.\n"
    "- Im Manus-Projektkontext die helle Logo-Datei als `logo_white.png` ablegen. "
    "Skript-Aufruf zwingend mit `--project-dir .`.\n"
    "- Prüfe nach dem Generieren, dass das Logo im Footer jeder Folie sichtbar ist.\n"
    "\n"
    "**ALLE 7 FOLIEN – PFLICHT:**\n"
    "- Lade JEDE der 7 HTML-Folien (slide_01.html bis slide_07.html) "
    "nacheinander per `slide_edit`. KEINE Folie ueberspringen.\n"
...
def build_talent_report_prompt(extraction: ExtractionResult) -> str:
    ...
    prompt += TALENT_REPORT_POSTAMBLE
